...
def sync_to_firebase(version, message):
    ...
    config_path = os.path.join(PROJECT_ROOT, "remote_config.json")

    # Note: remoteconfig (no hyphen)
    fetch_cmd = f"firebase remoteconfig:get -o remote_config.json --project {PROJECT_ID}"
...
def show_menu():
    while True:
        ...
        print("3. 🛡️  Deploy Firestore Rules")
        print("4. ☢️  Nuclear Clean Functions")
        print("5. 📜 View Scraper Logs")
        print("6. 📦 Install App Dependencies")
        print(f"{Colors.BLUE}--- Deployment ---{Colors.ENDC}")
        print(f"7. 🧠 {Colors.BOLD}SMART DEPLOY (Auto-everything){Colors.ENDC}")
        print("8. 🏗️  MANUAL APK BUILD (Force Full Build)")
        print("9. 📜 View EAS Build Status")
        print("0. ❌ Exit")
        print(f"{Colors.HEADER}========================================{Colors.ENDC}")
        ...
        if choice == "1": run_command("npx expo start -c")
            ...
            input("\nPress Enter...")
        elif choice == "3": run_command("firebase deploy --only firestore:rules", cwd=FUNCTIONS_DIR); input("\nPress Enter...")
        elif choice == "4":
            shutil.rmtree(os.path.join(FUNCTIONS_DIR, "node_modules"), ignore_errors=True)
            run_command("npm install", cwd=FUNCTIONS_DIR)
        elif choice == "5": run_command("firebase functions:log --only scrapeProduct"); input("\nPress Enter...")
        elif choice == "6": run_command("npm install"); input("\nPress Enter...")
        elif choice == "7": smart_deploy(); input("\nPress Enter...")
        elif choice == "8": manual_apk_build(); input("\nPress Enter...")
        elif choice == "9": run_command("eas build:list --limit 5"); input("\nPress Enter...")
